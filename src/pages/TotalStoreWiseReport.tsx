...
const TotalStoreWiseReport: React.FC = () => {
  const { colors, theme } = useTheme();
  const [selectedYear, setSelectedYear] = useState("2018");
  const [selectedStore, setSelectedStore] = useState("All Stores");

  // Mock Data mimicking the layout of Product Wise Report but for Store Wise
  const months = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  const reportData = [
    { month: "01", name: "January", amount: 0 },
    { month: "02", name: "February", amount: 15.50 },
    { month: "03", name: "March", amount: 120.45 },
    { month: "04", name: "April", amount: 85.00 },
    { month: "05", name: "May", amount: 0 },
    { month: "06", name: "June", amount: 0 },
    { month: "07", name: "July", amount: 0 },
    { month: "08", name: "August", amount: 45.20 },
  ...
  const maxAmount = 150; // For chart scaling
    ...
    <>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "24px", paddingBottom: "60px", animation: "fadeUp 0.5s cubic-bezier(0.16, 1, 0.3, 1)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: "12px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", border: `1px solid ${colors.border}`, borderRadius: "8px", overflow: "hidden", backgroundColor: colors.card }}>
            <span style={{ padding: "8px 12px", fontSize: "14px", fontWeight: 700, color: colors.textMain, backgroundColor: colors.bg, borderRight: `1px solid ${colors.border}` }}>Store</span>
            <select 
              value={selectedStore}
            ...
            >
              ...
              <option value="SymboSys Main">SymboSys Main</option>
            </select>
          </div>
        ...
        <div style={{ backgroundColor: colors.card, borderRadius: "16px", border: `1px solid ${colors.border}`, overflow: "hidden", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.02)" }}>
          <div style={{ padding: "16px 24px", borderBottom: `1px solid ${colors.border}` }}>
            <h2 style={{ fontSize: "16px", fontWeight: 700, color: colors.textMain, margin: 0 }}>Total Paid Orders (Store Wise) - Report</h2>
          </div>
          <div style={{ padding: "32px 24px 24px 24px" }}>
            <div style={{ display: "flex", height: "300px", position: "relative" }}>
              
              {/* Y-Axis Labels */}
              <div style={{ display: "flex", flexDirection: "column", justifyContent: "space-between", paddingRight: "16px", borderRight: `1px solid ${colors.border}`, color: colors.textMuted, fontSize: "12px", fontWeight: 600, textAlign: "right", width: "40px" }}>
              ...
              <div style={{ flex: 1, position: "relative", marginLeft: "16px", display: "flex", alignItems: "flex-end", justifyContent: "space-between" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", zIndex: 0, pointerEvents: "none" }}>
                   {[...Array(6)].map((_, i) => (
                    <div key={i} style={{ width: "100%", height: "1px", backgroundColor: colors.border, opacity: i === 5 ? 0 : 0.5 }} />
                  ))}
                </div>
                ...
                {reportData.map((data) => (
                  <div key={data.month} className="chart-bar" style={{ display: "flex", flexDirection: "column", alignItems: "center", flex: 1, zIndex: 1, position: "relative", height: "100%", justifyContent: "flex-end" }}>
                    ...
                    <div style={{ 
                      ...
                      backgroundColor: data.amount > 0 ? colors.primary : "transparent",
                      borderBottom: data.amount === 0 ? `2px solid ${colors.primary}` : "none",
        ...
        <div style={{ backgroundColor: colors.card, borderRadius: "16px", border: `1px solid ${colors.border}`, overflow: "hidden", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.02)" }}>
          <div style={{ padding: "16px 24px", borderBottom: `1px solid ${colors.border}` }}>
            <h2 style={{ fontSize: "16px", fontWeight: 700, color: colors.textMain, margin: 0 }}>Total Paid Orders (Store Wise) - Report Data</h2>
          </div>
                ...
                {reportData.map((data, idx) => (
                  <tr key={data.month} style={{ borderBottom: idx === reportData.length - 1 ? "none" : `1px solid ${colors.border}`, backgroundColor: theme === "light" ? (idx % 2 === 0 ? "transparent" : "#f8fafc") : (idx % 2 === 0 ? "transparent" : "#0f172a") }}>
                    ...
                    <td style={{ padding: "16px 24px", fontSize: "14px", fontWeight: 600, color: colors.textMain }}>
                      ${data.amount === 0 ? "0" : data.amount.toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
...
export default TotalStoreWiseReport;
